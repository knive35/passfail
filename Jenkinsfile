...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/eonsmartcamera-dev-apps/${MS_NAME}:eonsmartcamera-dev-apps --dry-run -o yaml >> Orchestration/deployment-eonsmartcamera-dev.yaml'
